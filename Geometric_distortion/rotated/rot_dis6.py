...
if __name__ == "__main__":
    ...
    if o_count < 6:
        raise ValueError("氧原子数量不足（<6），无法按照要求对第6个O原子进行旋转。")
    ...
    # 使用前4个O定义平面，并选取第6个O为目标旋转原子
    O1, O2, O3, O4 = O_list[0], O_list[1], O_list[2], O_list[3]
    O_target = O_list[5]  # 第6个O原子（索引5）
    ...
    for angle in range(0, 61):
        ...
        # 输出文件名带角度标记
        output_filename = f"IrO6_modified_6thO_{angle}deg.vasp"
    ...
    print("已生成0-60度旋转并缩短距离后的第6个O原子坐标文件。")
